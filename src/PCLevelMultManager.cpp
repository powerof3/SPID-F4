...
namespace PCLevelMult
{
	...
	{
		npcFormID = a_base->IsCreated() ? a_character->GetFormID() : a_base->GetFormID();  // use character formID for permanent storage
	}

	void Manager::Register()
	{
		if (const auto UI = RE::UI::GetSingleton()) {
			UI->RegisterSink(GetSingleton());
	...
	void Manager::DumpRejectedEntries()
	{
		...
		for (auto& [playerID, npcFormIDs] : _cache) {
			...
			for (auto& [npcFormID, levelMap] : npcFormIDs) {
				logger::info("\tNPC : {} [{:X}]", RE::TESForm::GetFormByID(npcFormID)->GetFormEditorID(), npcFormID);
				for (auto& [level, distFormMap] : levelMap.entries) {
					logger::info("\t\tLevel : {}", level);
					for (auto& [distFormID, idxSet] : distFormMap.rejectedEntries) {
						logger::info("\t\t\tDist FormID : {} [{:X}]", RE::TESForm::GetFormByID(distFormID)->GetFormEditorID(), distFormID);
						for (auto& idx : idxSet) {
							logger::info("\t\t\t\tIDX : {}", idx);
						}
					}
				...
				for (auto& [level, distFormMap] : levelMap.entries) {
					...
					for (auto& [formType, formIDSet] : distFormMap.distributedEntries) {
						...
						for (auto& formID : formIDSet) {
							logger::info("\t\t\t\tDist FormID : {} [{:X}]", RE::TESForm::GetFormByID(formID)->GetFormEditorID(), formID);
						}
					}
				}
			}
