...
#pragma region Parsing
	namespace INI
	{
		...
		bool TryParse(const std::string& key, const std::string& value, const Path& path)
		{
			...
			} catch ([[maybe_unused]] const Exception::MissingComponentParserException& e) {
...
#pragma region Lookup
		...
		for (auto& rawForm : genericForms) {
			if (auto form = detail::LookupLinkedForm(dataHandler, rawForm); form) {
				...
				if (const auto keyword = form->As<RE::BGSKeyword>(); keyword) {
					...
					if (formType == RE::FormType::kPACK || formType == RE::FormType::kFLST) {
						...
						logger::warn("\t[{}] Unsupported Form type: {}", path, formType);
					}
				}
			}
		}
	...
	{
		ForEachLinkedForms([&]<typename Form>(LinkedForms<Form>& a_linkedConfigs) {
			if (a_linkedConfigs.forms[type].empty()) {
				return;
			}
			std::unordered_map<RE::TESForm*, std::vector<DistributedForm>> map{};

			// Iterate through the original map
			for (const auto& [path, formsMap] : a_linkedConfigs.forms[type]) {
			...
			const auto& recordName = RECORD::GetTypeName(a_linkedConfigs.GetType());
