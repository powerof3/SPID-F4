		...
		std::variant<Form*, const RE::TESFile*> get_form_or_mod(RE::TESDataHandler* const dataHandler, const FormOrEditorID& formOrEditorID, const Path& path, const LookupOptions options)
		{
			...
			auto find_or_create_keyword = [&](const std::string& editorID) -> RE::BGSKeyword* {
				...
				if (result != keywordArray.end()) {
					...
					const auto factory = RE::ConcreteFormFactory<RE::BGSKeyword>::GetFormFactory();
			...
			std::visit(overload{
						   [&](const FormModPair& formMod) {
							   ...
							   if (formID) {
								   ...
								   if (modName) {
									   ...
									   anyForm = RE::TESForm::GetFormByID(*formID);
								   }
								   ...
								   if (!form) {
									   throw MismatchingFormTypeException(Form::FORM_ID, anyForm->GetFormType(), FormModPair{ *formID, modName }, path);
								   }
						   ...
						   [&](const std::string& editorID) {
							   ...
							   if constexpr (std::is_same_v<Form, RE::BGSKeyword>) {
								   ...
								   if (const auto anyForm = RE::TESForm::GetFormByEditorID(editorID); anyForm) {
									   form = as_form(anyForm);
									   if (!form) {
										   throw MismatchingFormTypeException(anyForm->GetFormType(), Form::FORM_ID, editorID, path);
									   }
								   } else {
...
{
	if (isValid) {
		forms.emplace_back(static_cast<std::uint32_t>(forms.size()), form, idxOrCount, filters, path);
	}
	lookupCount++;
}
...
void Forms::LookupGenericForm(RE::TESDataHandler* const dataHandler, Distribution::INI::Data& rawForm, std::function<void(bool isValid, Form*, const IndexOrCount&, const FilterData&, const Path& path)> callback)
{
	...
	try {
		if (auto form = detail::get_form<Form>(dataHandler, formOrEditorID, path, LookupOptions::kCreateIfMissing); form) {
			...
			for (auto& [av, range] : level.avLevels) {
				auto avID = std::get<FormOrEditorID>(av);
				try {
					av = detail::get_form<RE::ActorValueInfo>(dataHandler, avID, path, LookupOptions::kNone);
				} catch (const Lookup::UnknownFormIDException& e) {
					buffered_logger::error("\t\t[{}] [0x{:X}] ({}) ActorValue FAIL - formID doesn't exist", e.path, e.formID, e.modName.value_or(""));
				} catch ([[maybe_unused]] const Lookup::InvalidKeywordException& e) {
					// Not a keyword
				} catch ([[maybe_unused]] const Lookup::KeywordNotFoundException& e) {
					// Not a keyword
				} catch (const Lookup::UnknownEditorIDException& e) {
					buffered_logger::error("\t\t[{}] ({})  ActorValue FAIL - editorID doesn't exist", e.path, e.editorID);
				} catch (const Lookup::MalformedEditorIDException& e) {
					buffered_logger::error("\t\t[{}] ActorValue FAIL - editorID can't be empty", e.path);
				} catch (const Lookup::MismatchingFormTypeException& e) {
					std::visit(overload{
								   [&](const FormModPair& formMod) {
									   auto& [formID, modName] = formMod;
									   buffered_logger::error("\t\t[{}] [0x{:X}] ({}) ActorValue FAIL - mismatching form type (expected: {}, actual: {})", e.path, *formID, modName.value_or(""), e.expectedFormType, e.actualFormType);
								   },
								   [&](std::string editorID) {
									   buffered_logger::error("\t\t[{}] ({}) ActorValue FAIL - mismatching form type (expected: {}, actual: {})", e.path, editorID, e.expectedFormType, e.actualFormType);
								   } },
						e.formOrEditorID);
